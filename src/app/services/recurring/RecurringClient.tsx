...
export default function RecurringClient({ recurringPrices, displayBedrooms }: RecurringClientProps) {
  const [bedrooms, setBedrooms] = useState(displayBedrooms);
  const [freq, setFreq] = useState('biweekly');

  const perVisit = recurringPrices[freq]?.[bedrooms] ?? 0;
  const oneTimeRate = recurringPrices.onetime?.[bedrooms] ?? 0;
  const visits = visitsPerYear[freq];
  const annual = perVisit * visits;
  const wouldBe = oneTimeRate * visits;
  const saved = wouldBe - annual;

  // Plan card prices for the display bedroom count
  const monthlyPrice = recurringPrices.monthly?.[displayBedrooms] ?? 0;
  const biweeklyPrice = recurringPrices.biweekly?.[displayBedrooms] ?? 0;
  const weeklyPrice = recurringPrices.weekly?.[displayBedrooms] ?? 0;
  const onetimePrice = recurringPrices.onetime?.[displayBedrooms] ?? 0;

  // Available bedroom options from pricing data
  const bedroomOptions = Object.keys(recurringPrices.onetime ?? {}).map(Number).sort((a, b) => a - b);
        ...
        <div className="max-w-[1200px] mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-5">
              ...
              <div className="bg-warm-white border-2 border-cream-dark rounded-2xl p-9 flex flex-col transition-all duration-300">
                ...
                <div className="flex items-baseline gap-1.5 mb-1">
                  <span className="font-display font-semibold text-[42px] text-forest tracking-tight leading-none">${monthlyPrice}</span>
                  <span className="text-[15px] text-gray-m font-medium">/visit</span>
                </div>
                <div className="text-[13px] text-gray-m mb-6 line-through">${onetimePrice} one-time</div>
              ...
              <div className="bg-warm-white border-2 border-forest rounded-2xl p-9 relative flex flex-col shadow-lg transition-all duration-300">
                ...
                <div className="flex items-baseline gap-1.5 mb-1">
                  <span className="font-display font-semibold text-[42px] text-forest tracking-tight leading-none">${biweeklyPrice}</span>
                  <span className="text-[15px] text-gray-m font-medium">/visit</span>
                </div>
                <div className="text-[13px] text-gray-m mb-6 line-through">${onetimePrice} one-time</div>
              ...
              <div className="bg-warm-white border-2 border-cream-dark rounded-2xl p-9 flex flex-col transition-all duration-300">
                ...
                <div className="flex items-baseline gap-1.5 mb-1">
                  <span className="font-display font-semibold text-[42px] text-forest tracking-tight leading-none">${weeklyPrice}</span>
                  <span className="text-[15px] text-gray-m font-medium">/visit</span>
                </div>
                <div className="text-[13px] text-gray-m mb-6 line-through">${onetimePrice} one-time</div>
          ...
          <Reveal delay={200}>
            <p className="font-body text-sm text-gray-m text-center max-w-[600px] mx-auto leading-relaxed">
              Prices shown are for a {displayBedrooms}BR/2BA home — Tacoma&apos;s most common configuration. Your exact rate depends on home size and condition.{' '}
